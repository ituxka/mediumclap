...
const PR_ROOT = 'https://github.com/ohansemmanuel/advanced-react-patterns-ultrasimplified/pull';
  ...
  return (
    <StyledHeader>
      ...
      <div ref={setRef} data-refkey="lightBulbEl">
        <InfoIllustration style={{ width: '30px', marginLeft: '5px', cursor: 'pointer' }} onClick={toggleInfo} />
      </div>
    </StyledHeader>
  );
};
...
const RouteComponent = ({ pattern, patternNumber, isMediumOrLarger }) => {
  ...
  const beforeAndAfterPatternNumbers = [patternNumber, patternNumber + 1].map(indexToTwoDigits);

  // Demo to be shown in Display Boxes
  let Demo1, Demo2;
  try {
    Demo1 = require(`./patterns/${beforeAndAfterPatternNumbers[0]}.jsx`).default;
  } catch (error) {
    Demo1 = () => null;
  }
  try {
    Demo2 = require(`./patterns/${beforeAndAfterPatternNumbers[1]}.jsx`).default;
  } catch (error) {
    Demo2 = () => null;
  }

  const goToCodeImplementation = () => {
      ...
      <Columns>
        ...
        <Column leftGap>
          ...
          <CTAContainer mtop={!isMediumOrLarger && '10px'}>
            <Button text="Code Implementation" Icon={<GithubLogo style={{ width: '18px' }} />} onClick={goToCodeImplementation} />
          </CTAContainer>
        </Column>
      </Columns>
    </StyledContentContainer>
    ...
    <StyledAppBody>
      <Router>
        ...
        {patterns.map((pattern, index) => (
          <RouteComponent path={pattern} key={pattern} pattern={pattern} patternNumber={index + 1} isMediumOrLarger={isMediumOrLarger} />
        ))}
      </Router>
      {!isMediumOrLarger && <StyledFloatingBtn onClick={toggleSidebar}>+</StyledFloatingBtn>}
    </StyledAppBody>
  );
};
