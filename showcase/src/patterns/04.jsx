...
const MediumClap = ({ children, onClap, style: userStyle = {} }) => {
  ...
  return (
    <Provider value={clapStateM}>
      <button style={{ ...userStyle }} ref={clapAnimation.scaleRef} onClick={handleClapClick} className={styles.clap} type="button">
        {children}
      </button>
    </Provider>
  );
...
const Usage = () => {
  ...
  return (
    <div style={{ width: '100%' }}>
      <MediumClap style={{ border: '1px solid red' }} onClap={handleClap}>
        <ClapIcon />
        <ClapCount />
        <ClapCountTotal />
      </MediumClap>
