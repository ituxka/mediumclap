...
const withClapAnimation = (WrappedComponent) => {
  const WithClapAnimation = (props) => {
    const [animationTimeline] = useState(new mojs.Timeline());
...
const MediumClap = ({ animationTimeline }) => {
  ...
  return (
    <button onClick={handleClapClick} className={styles.clap}>
...
const ClapCount = ({ count }) => <span className={styles.count}>+{count}</span>;

const CountTotal = ({ countTotal }) => <span className={styles.total}>{countTotal}</span>;
